...
def analyze_strategy(df, t):
    try:
        ...
        # (1) [cite_start]季線多空與反彈 [cite: 4-16]
        ...
        if (curr_p - prev_p)/prev_p >= 0.05 and curr_v > prev_v * 1.5: 
            msg.append(f"🔥 強勢反彈：慎防破前三收盤 {p3_close:.1f}"); is_mail = True
        
        # (2) [cite_start]底部轉折與年線 [cite: 17-21, 32-36]
        up_tags = []
        ...
        if len(post_a) > 5:
            ...
            if len(post_b) > 3:
                ...
                if t_c_v >= (t_a_v * 0.97) and (w_p_v - t_a_v)/t_a_v >= 0.10:
                    ...
                    msg.append(f"{st_w}: 中間峰值頸線({w_p_v:.1f}) 領口距{((w_p_v-curr_p)/w_p_v)*100:.1f}%"); is_mail = True
        ...
        if t in ["2408", "2344"] and curr_p > v60: # 高檔股判讀 M頭
            ...
            if len(post_pk) > 5:
                ...
                msg.append(f"⚠️ M頭警示: 中間谷底價({m_v_v:.1f}) 提防跌破頸線"); is_mail = True

        # (4) [cite_start]多空壓力標記 [cite: 37-39, 48-51]
        ...
        if curr_p < v240: msg.append(f"❄️ 空方反彈中: 壓力240SMA({v240:.1f})")
            ...
            msg.append(f"🌊 多方行進: 近期壓力高點({p_high:.1f})")
...
if submit_btn:
    ...
    except: st.warning("雲端同步失敗，請檢查 Secrets 設定")
