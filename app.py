...
def analyze_strategy(df, t):
    try:
        ...
        # 💡 大盤專屬處理：強制發信並附上均線數據
        if t == "^TWII":
            msg.insert(0, f"📊 均線指標: 10SMA({v10:.1f}) / 20SMA({v20:.1f}) / 60SMA({v60:.1f})")
            is_mail = True
    ...
    for t in tickers:
        ...
        if not df.empty:
            ...
            with st.container(border=True):
                st.markdown(f"#### {STOCK_NAMES.get(t, t)} {t} - ${p:.2f} 乖離(SMA60) {b:.1f}%")
                st.write(sig)
            
            # 💡 信件內容：加入 乖離(SMA60) 標示
            if im:
                notify_list.append(f"【{STOCK_NAMES.get(t, t)}】${p:.2f} 乖離(SMA60) {b:.1f}% | {sig}")
