...
def analyze_strategy(df):
    try:
        ...
        if up_c >= 2 and curr_p < v60 and curr_p < v240: msg.append(f"✨ 底部轉折：{up_c}線翻揚"); is_mail = True
        # 4. 量價背離
        if curr_v > prev_v * 1.2 and curr_p < v5 and curr_p < prev_p: msg.append(f"⚠️ 量價背離，基準 {p3_close:.1f}"); is_mail = True
        # 5. 年線保衛
        if abs(curr_p - v240)/v240 < 0.05: msg.append("⚠️ 年線保衛戰"); is_mail = True
        # 6. 均線糾結
        if max(v5, v10, v20) / min(v5, v10, v20) < 1.02: msg.append("🌀 均線糾結"); is_mail = True
        # 7. 乖離率
        if curr_p > v60 * 1.3: msg.append(f"⚠️ 乖離過高({v60:.1f})"); is_mail = True
    ...
    for t in tickers:
        ...
        if not df.empty:
            ...
            with st.container(border=True):
                st.markdown(f"#### {STOCK_NAMES.get(t, t)} {t} - ${
