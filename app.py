...
def analyze_strategy(df, ticker):
    try:
        ...
        if len(up_tags) >= 2 and curr_p < v60 and curr_p < v240:
        ...
        if curr_p > v60 and curr_p > v240:
            if ticker in ["2408", "2344"]: # 高檔股判讀
                msg.append(f"⚠️ 高檔 M 頭警示：壓力高點({p_high:.1f})"); is_mail = True
            else:
                msg.append(f"🌊 多方行進，近期壓力({p_high:.1f})")
        elif curr_p < v240:
            msg.append(f"❄️ 空方反彈行進中，壓力240SMA({v240:.1f})")
