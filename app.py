...
# 🔧 系統設定
# ==========================================
st.set_page_config(page_title="股市戰略 - 糾結突破版", layout="wide")
...
def analyze_strategy(df):
    ...
    # ====== 新增戰略：均線糾結 ======
    # 為了精準判斷是「突破/跌破糾結區」，我們使用「昨日的均線」來判定糾結狀態
    ...
    if curr_price < v60 and curr_price > v240:
        # 避免與糾結突破重複顯示
        if not any("糾結" in m for m in messages):
...
def fetch_all_data(user_tickers):
    ...
    for t in user_tickers:
        ...
        if df.empty:
            processed_results.append({"code": t, "name": STOCK_NAMES.get(t, t), "error": "無資料或未上市"})
            continue
...
if submit_btn:
    ...
    st.info(f"正在掃描 {len(user_tickers)} 檔股票 (含均線糾結雷達)...")
