...
# --- 2. 核心分析邏輯 (100% 遵照條件判讀.docx 與 W底三點定標) ---
def analyze_strategy(df):
    try:
        ...
        # (2) [cite_start]強勢反彈 [cite: 15, 16]
        if (curr_p - prev_p)/prev_p >= 0.05 and curr_v > prev_v * 1.5: 
            msg.append(f"🔥 強勢反彈 (爆量漲5%) 慎防破 {p3_close:.1f}"); is_mail = True
            
        # (3) [cite_start]底部/高檔轉折 [cite: 20, 25]
        ...
        # (4) [cite_start]量價背離 [cite: 30, 31]
        if curr_v > prev_v * 1.2 and curr_p < v5 and curr_p < prev_p:
            msg.append(f"⚠️ 量價背離 (量增價跌) 基準價 {p3_close:.1f}"); is_mail = True

        # (5) [cite_start]年線保衛戰 [cite: 35, 38]
        if abs(curr_p - v240)/v240 < 0.05: msg.append("⚠️ 年線保衛戰"); is_mail = True
        elif curr_p < v240 and dn_c >= 2: msg.append("❄️ 空方弱勢整理"); is_mail = True

        # (6) [cite_start]均線糾結 [cite: 42]
        if max(v5, v10, v20) / min(v5, v10, v20) < 1.02: msg.append("🌀 均線糾結：變盤在即"); is_mail = True

        # (7) [cite_start]乖離率過高 [cite: 46]
        if curr_p > v60 * 1.3: msg.append(f"⚠️ 乖離過高：60SMA({v60:.1f})"); is_mail = True
        ...
        if len(post_a) > 5:
            ...
            if len(post_b) > 3:
                ...
                if t_c_v >= (t_a_v * 0.97) and (w_p_v - t_a_v)/t_a_v >= 0.10:
                    ...
                    msg.append(f"{status}: 左底({a_d}日前), 頸高({b_d}日前), 領口距 {((w_p_v-curr_p)/w_p_v)*100:.1f}%")
                    is_mail = True
...
if submit_btn:
    ...
    try:
        ...
        sheet.update_cell(cell.row, cell.col + 2, now_str) # 💡 落實時間更新
