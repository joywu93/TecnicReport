...
def analyze_strategy(df):
    try:
        ...
        # [cite_start]2. 強勢反彈 [cite: 15, 16]
        if (curr_p - prev_p)/prev_p >= 0.05 and curr_v > prev_v * 1.5: 
            msg.append(f"🔥 強勢反彈：爆量漲停，慎防未來跌破 {p3_close:.1f}"); is_mail = True

        # [cite_start]3. 底部轉折 / 高檔整理 [cite: 20, 25]
        ...
        if up_c >= 2 and curr_p < v60 and curr_p < v240: msg.append(f"✨ 底部轉折：{up_c}均線翻揚 60MA({v60:.1f})"); is_mail = True
        elif dn_c >= 2 and curr_p > v60 and curr_p > v240 and curr_p < v5: msg.append(f"✨ 高檔轉整理：{dn_c}均線翻下 5MA({v5:.1f})"); is_mail = True

        # [cite_start]4. 量價背離 [cite: 30, 31]
        if curr_v > prev_v * 1.2 and curr_p < v5 and curr_p < prev_p:
            msg.append(f"⚠️ 量價背離：量增價跌破5MA({v5:.1f}) 觀察基準 {p3_close:.1f}"); is_mail = True

        # [cite_start]5. 年線保衛戰 [cite: 35, 38]
        if abs(curr_p - v240)/v240 < 0.05 and dn_c >= 2: msg.append("⚠️ 年線保衛戰：均線偏弱提防長黑"); is_mail = True
        elif curr_p < v240 and dn_c >= 2: msg.append("❄️ 空方弱勢整理：均線蓋頭"); is_mail = True

        # [cite_start]6. 均線糾結 [cite: 42]
        if max(v5, v10, v20) / min(v5, v10, v20) < 1.02: msg.append("🌀 均線糾結：變盤在即"); is_mail = True

        # [cite_start]7. 乖離率過高 [cite: 46]
        if curr_p > v60 * 1.3: msg.append(f"⚠️ 乖離過高：60MA({v60:.1f})"); is_mail = True
