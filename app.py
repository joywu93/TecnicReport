...
# --- 1. 112 檔完整名單 (解決 image_589f70.png 缺失問題) ---
...
def analyze_strategy(df):
    try:
        ...
        # [cite_start]2. 強勢反彈 [cite: 15]
        if (curr_p - prev_p)/prev_p >= 0.05 and curr_v > prev_v * 1.5: msg.append(f"🔥 強勢反彈 (漲>5%且爆量1.5倍) 慎防破前三收盤 {p3_close:.1f}"); is_mail = True

        # [cite_start]3. 底部/高檔轉折 [cite: 20, 25]
        ...
        if up_c >= 2 and curr_p < v60 and curr_p < v240: msg.append(f"✨ 底部轉折：{up_c}條均線翻揚"); is_mail = True
        elif dn_c >= 2 and curr_p > v60 and curr_p > v240 and curr_p < v5: msg.append(f"✨ 高檔轉整理：{dn_c}條均線翻下"); is_mail = True

        # [cite_start]4. 量價背離 [cite: 30]
        if curr_v > prev_v * 1.2 and curr_p < v5 and curr_p < prev_p: msg.append(f"⚠ 量價背離 (量增價跌) 基準價 {p3_close:.1f}"); is_mail = True

        # [cite_start]5. 年線保衛戰 [cite: 35]
        if abs(curr_p - v240)/v240 < 0.05 and dn_c >= 2: msg.append("⚠ 年線保衛戰：提防長黑"); is_mail = True

        # [cite_start]6. 均線糾結 [cite: 42]
        if max(v5, v10, v20) / min(v5, v10, v20) < 1.02: msg.append("🌀 均線糾結：變盤在即"); is_mail = True

        # [cite_start]7. 乖離率 [cite: 46]
        ...
        [cite_start]if not msg: msg.append("🌊 多方行進" if curr_p > v60 else "☁ 空方盤整") # [cite: 50, 51]
...
if submit_btn:
    ...
    try:
        ...
        sheet.update_cell(cell.row, cell.col + 2, now_str) # 💡 同步更新時間
    ...
    for t in tickers:
        ...
        if not df.empty:
            ...
            with st.container(border=True):
                st.markdown(f"#### {STOCK_NAMES.get(t, t)} {t} - ${p:.2f} 乖離率 {b:.1f}%")
                st.write(sig)
