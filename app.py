...
def analyze_strategy(df):
    try:
        ...
        ma60 = close.rolling(60).mean(); ma240 = close.rolling(240).mean()
        ...
        # 1. 季線轉折 
        if prev_p < v60 and curr_p > v60: msg.append(f"🚀 轉多訊號：站上季線({v60:.1f})"); is_mail = True
        elif prev_p > v60 and curr_p < v60: msg.append(f"📉 轉空警示：跌破季線({v60:.1f})"); is_mail = True

        # 2. 強勢反彈 
        if (curr_p - prev_p)/prev_p >= 0.05 and curr_v > prev_v * 1.5: 
            msg.append(f"🔥 強勢反彈 (爆量且漲>5%) 慎防跌破前三日收盤 {p3_close:.1f}"); is_mail = True

        # 3. 底部轉折 / 高檔整理 
        up_count = sum([ma5.diff().iloc[-1]>0, ma10.diff().iloc[-1]>0, ma20.diff().iloc[-1]>0])
        down_count = sum([ma5.diff().iloc[-1]<0, ma10.diff().iloc[-1]<0, ma20.diff().iloc[-1]<0])
        if up_count >= 2 and curr_p < v60 and curr_p < v240: msg.append(f"✨ 底部轉折：{up_count}條均線翻揚"); is_mail = True
        elif down_count >= 2 and curr_p > v60 and curr_p > v240 and curr_p < v5: msg.append(f"✨ 高檔轉整理：{down_count}條均線翻下"); is_mail = True

        # 4. 量價背離 
        if curr_v > prev_v * 1.2 and curr_p < v5 and curr_p < prev_p:
            msg.append(f"⚠ 量價背離 (量增價跌) 慎防假跌破基準 {p3_close:.1f}"); is_mail = True

        # 5. 年線保衛戰 
        if abs(curr_p - v240)/v240 < 0.05 and down_count >= 2: msg.append("⚠ 年線保衛戰：提防長黑"); is_mail = True

        # 6. 均線糾結 
        if max(v5, v10, v20) / min(v5, v10, v20) < 1.02: msg.append("🌀 均線糾結：變盤在即"); is_mail = True

        # 7. 乖離率過高 
        if curr_p > v60 * 1.3: msg.append(f"⚠ 乖離率過高60SMA({v60:.1f})"); is_mail = True
        ...
        if len(post_a) > 5:
            ...
            if len(post_b) > 3:
                ...
                if t_c_v >= (t_a_v * 0.97) and (w_p_v - t_a_v)/t_a_v >= 0.10:
                    ...
                    msg.append(f"{status}(距領口{gap:.1f}%)")
                    is_mail = True

        if not msg: msg.append("🌊 多方行進" if curr_p > v60 else "☁ 空方盤整") # [cite: 48-51]
...
if submit_btn:
    ...
    try:
        ...
        sheet.update_cell(cell.row, cell.col + 2, now_str) # 💡 更新時間
