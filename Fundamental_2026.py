...
st.title("📊 2026 戰略指揮 (V176 回歸初心穩定版)")
...
cached_data = fetch_gsheet_data_v176()
...
# 4. 執行與呈現
# ==========================================
# 💡 V176: 斷尾求生純淨版！完全捨棄 DataFrame 樣式引擎，100% 絕對防當機！
def render_dataframe(df_source, is_finance=False, is_single=False):
    ...
    # 5. 強制將所有數值純淨文字化 (完全放棄 Pandas 內建的 .format 與 .style)
    for c in df.columns:
        if c != "股票名稱":
            # 先轉成數字，不行的變成 0.0
            ...
            # 手動轉成純文字格式
            if "(%)" in c or "%" in c:
                df[c] = df[c].apply(lambda x: f"{x:.2f}%" if pd.notna(x) and not math.isinf(x) else "-")
            elif "次數" in c:
                df[c] = df[c].apply(lambda x: f"{int(x)}" if pd.notna(x) and not math.isinf(x) else "-")
            else:
                df[c] = df[c].apply(lambda x: f"{x:.2f}" if pd.notna(x) and not math.isinf(x) else "-")
                
    calc_height = None if is_single else (800 if is_finance else 600)
    
    # 🛡️ 6. 終極安全輸出：這只是一個裝滿普通文字的 DataFrame，神仙來了都不會報錯！
                ...
                with c1:
                    ...
                    if sel:
                        ...
                        if not row_df.empty:
                            ...
                            # 🛡️ 終極純淨提取：強制將可能出錯的資料型態壓平
                            def get_safe_scalar(val):
                                if isinstance(val, pd.Series): return val.iloc[0]
                                if isinstance(val, dict) or isinstance(val, list): return 0.0
                                try: return float(val)
                                except: return 0.0
                                
                            liab_value = get_safe_scalar(row.get('最新季度流動合約負債(億)', 0)) 
                            liab_qoq = get_safe_scalar(row.get('最新季度流動合約負債季增(%)', 0))
                            safe_price = get_safe_scalar(row.get('最新股價', 0))
                            safe_yield = get_safe_scalar(row.get('前瞻殖利率(%)', 0))
                            safe_per = get_safe_scalar(row.get('本益比(PER)', 0))
                            safe_eps = get_safe_scalar(row.get('預估今年度_EPS', 0))
                            safe_grow = get_safe_scalar(row.get('預估年成長率(%)', 0))
