...
# 💡 更新內容： 還原穩定版資料讀取邏輯，確保與舊表單 100% 吻合
# ==========================================
...
# 🌟 核心快取大腦 
# ==========================================
@st.cache_data(ttl=3600, show_spinner="連線至雙核大數據庫 (穩定版)...")
...
if cached_data and "error" in cached_data:
    st.error(f"檔案解析失敗，可能是表單欄位名稱異動。詳細錯誤：{cached_data['error']}")
    cached_data = None

# ==========================================
# 側邊欄：登入與動態權限判斷
# ==========================================
if st.sidebar.button("🔄 重新載入最新表單資料 (清除雲端快取)", type="primary", use_container_width=True):
    clear_cache_and_session()
    st.sidebar.success("✅ 雲端快取已清除！請稍候...")
    time.sleep(1)
    force_rerun()
            ...
            if st.button("🚀 執行戰略分析", type="primary", use_container_width=True):
                ...
                for i, code in enumerate(vips):
                    ...
                    if d:
                        ...
                        # 💡 呼叫時移除 this_year_q1_eps，確保完全相容舊表單
                        res_list.append(auto_strategic_model(f"{code} {d['name']}", simulated_month, d.get("rev_last_11",0), d.get("rev_last_12",0), d.get("rev_this_1",0), d.get("rev_this_2",0), d.get("rev_this_3",0), d["base_q_eps"], d.get("non_op",0), d["base_q_avg_rev"], d["ly_q1_rev"], d["ly_q2_rev"], d["ly_q3_rev"], d["ly_q4_rev"], d["y1_q1_rev"], d["y1_q2_rev"], d["y1_q3_rev"], d["y1_q4_rev"], d.get("payout",0), pr, d.get("contract_liab",0), d.get("contract_liab_qoq",0), d.get("acc_eps",0), d.get("declared_div",0), 0))
                bar.empty()
