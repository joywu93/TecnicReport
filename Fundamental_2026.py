...
#    3. 介面語法防呆升級：改用三重引號保護字串，杜絕 SyntaxError 報錯！
# ==========================================
    ...
    try:
        ...
        def parse_df(df):
            ...
            for idx, row in df.iterrows():
                ...
                # 🌟 V01 防彈特色：完全不理會爬蟲重算，直接抓取您表單中原有的「業外損益佔稅前淨利(%)」
                ...
                new_entry = {
                    ...
                    "rev_last_10": v(get_col(f"{last_y}M10", "營收", ex=["增", "率", "%"])), # 補上API版需要的10~6月動態欄位
                    ...
                    "base_q_total_rev": rev_q4 if rev_q4 > 0 else rev_q3, 
                    ...
                    if sel:
                        ...
                        if row_list: 
                            try:
                                ...
                                # 🌟 改用三重引號寫法，絕對不會再報 SyntaxError
                                st.markdown(f"""
                                    股價 {safe_price:.2f}元 ｜ 前瞻殖利率 {safe_yield:.2f}%<br>
                                    PER {safe_per:.2f} ｜ EPS {safe_eps:.2f}元 ｜ 成長率 {safe_grow:.2f}% ｜ 📉 業外佔比 {safe_non_op:.2f}%<br>
                                    📈 合約負債 {liab_value:.2f}億 ({liab_qoq:.2f}%)
                                """, unsafe_allow_html=True)
                ...
                with st.spinner("掃描中..."):
                    ...
                    for code, d in db_gen.items():
                        ...
                        r = auto_strategic_model(
                            ...
                            d["ly_q1_rev"], d["ly_q2_rev"], d["ly_q3_rev"], d["ly_q4_rev"], 
                            d["y1_q1_rev"], d["y1_q2_rev"], d["y1_q3_rev"], d["y1_q4_rev"], 
                            d.get("payout",0), d.get("price", 0), d.get("contract_liab",0), d.get("contract_liab_qoq",0), 
                            d.get("acc_eps",0), d.get("declared_div",0), d.get("actual_q1_eps",0)
                        )
