...
# 4. 執行與呈現
# ==========================================
# 💡 V163: 強制清洗隱藏 index，徹底根除 NaN 崩潰 Bug！
def render_dataframe(df_source, is_finance=False, is_single=False):
    if df_source.empty: return
    
    # 🛡️ 第一道防線：強制 reset_index，洗掉單一擷取時帶過來的殘留編號
    ...
    # 🛡️ 第二道防線：安全格式化，空值顯示為 - 而非 NaN
    st.dataframe(
        df.set_index("股票名稱").style.map(
            lambda v: 'color: #ff4b4b; font-weight: bold' if isinstance(v, (int, float)) and v >= (5.0 if is_finance else 4.0) else '', 
            subset=['前瞻殖利率(%)']
        ).format(f_dict, na_rep="-"), 
        height=calc_height, 
