...
st.title("📊 2026 戰略指揮 (V153 核彈穿透救援版)")
...
# 💡 V153 終極穿透版：完美偽裝 Header + TYPEK='all' (絕不被擋)
def get_mops_single_ytd_v153(code, y_roc, season):
...
if is_admin:
    ...
    # 💡 V153 核彈救援引擎：根據 EPS 或毛利空白進行精準補件
    if st.sidebar.button("⚡ 4️⃣ 季報缺件補完 (專補空白/單季還原)", type="secondary", use_container_width=True):
        with st.status("啟動 V153 核彈單季還原引擎...", expanded=True) as status:
            try:
                y_roc, q_num = str((2000 + int(target_q[:2])) - 1911), int(target_q[3])
                worksheets = get_gspread_client().open_by_url(MASTER_GSHEET_URL).worksheets()
                target_sheets = [ws for ws in worksheets if "個股總表" in ws.title or "金融股" in ws.title]
                    ...
                    if i_c != -1 and i_e != -1:
                        ...
                        if missing_codes:
                            if len(missing_codes) > 100:
                                st.warning(f"⚠️ {ws.title} 發現過多缺件！為避免封鎖，請先執行第3步極速清洗。")
                                missing_codes = missing_codes[:20]
                                
                            st.write(f"在 {ws.title} 鎖定 {len(missing_codes)} 檔缺件，潛入 MOPS 執行單季還原...")
                            for code in missing_codes:
                                q4_data = get_mops_single_ytd_v153(code, y_roc, q_num)
                                if q4_data:
                                    q3_data = get_mops_single_ytd_v153(code, y_roc, q_num - 1) if q_num > 1 else {"rev":0, "gp":0, "op":0, "eps":0}
                                    ...
                                    if sq_rev > 0:
                                        ...
                                        st.write(f"✅ 成功還原 {code}：毛利率 {round(gm, 2)}% | 營益率 {round(om, 2)}%")
                                        total_fixed += 1
                                        time.sleep(0.5) 
