...
st.title("📊 2026 戰略指揮 (V175 終極核彈裝甲版)")
...
# 🌟 核心快取大腦 (V175 徹底洗淨快取)
# ==========================================
@st.cache_data(ttl=3600, show_spinner="連線至雙核大數據庫 (啟動核彈級排毒)...")
def fetch_gsheet_data_v175():
    try:
...
cached_data = fetch_gsheet_data_v175()
...
if is_admin:
    ...
    if st.sidebar.button("⚡ 2️⃣ 官方月營收更新", type="secondary", use_container_width=True):
        with st.status(f"鎖定目標欄位【{auto_ym}】...", expanded=
