...
st.title("📊 2026 戰略指揮 (V165 終極排毒濾水器版)")
    ...
    try:
        ...
        def parse_df(df):
            ...
            for idx, row in df.iterrows():
                ...
                # 💡 V165 終極排毒濾水器：攔截 Google Sheet 裡的 NaN 與 inf 毒素！
                def v(c_name, d=0.0):
                    if not c_name or pd.isna(row[c_name]): return d
                    val_str = str(row[c_name]).replace(',', '').strip()
                    # 直接封殺所有的異常字眼
                    if not val_str or val_str.lower() in ['-', 'nan', 'inf', '-inf', 'infinity', '-infinity']: return d
                    try: 
                        val = float(val_str)
...
# 4. 執行與呈現
# ==========================================
# 💡 V165 鋼鐵防爆渲染機制：徹底消滅 nan% 與 inf% 視覺污染！
def render_dataframe(df_source, is_finance=False, is_single=False):
    if df_source.empty: return
    
    # 重置編號並淨化所有隱藏的 inf 或 -inf，將它們變回 NaN 讓引擎好處理
    ...
    def map_color(val):
        try:
            if pd.notna(val) and val is not None and str(val).strip() not in ['', '-']:
    ...
    if is_finance:
        ...
        f_dict = {c: "{:.2f}%" if "(%)" in c else ("{:.0f}" if "次數" in c else "{:.2f}") for c in df.columns if c != "股票名稱"}
    else:
        cols = ["股票名稱", "最新股價", "當季預估均營收", "季成長率(YoY)%", "前瞻殖利率(%)", "預估今年Q1_EPS", "預估今年度_EPS", "最新累季EPS", "本益比(PER)", "預估年成長率(%)", "運算配息率(%)", "最新季度流動合約負債(億)", "最新季度流動合約負債季增(%)"]
        df = df[[c for c in cols if c in df.columns]]
        f_dict = {c: "{:.2f}%" if "(%)" in c or "%" in c else "{:.2f}" for c in df.columns if c != "股票名稱"}
    
    # 🛡️ 遇到任何無效數字(NaN)，一律優雅顯示為 "-"
    st.dataframe(
        df.set_index("股票名稱").style.map(map_color, subset=['前瞻殖利率(%)']).format(f_dict, na_rep="-"), 
        height=calc_height, 
