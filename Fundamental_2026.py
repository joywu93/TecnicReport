# ==========================================
# 📂 檔案名稱： update_finance.py (V182 專注 EPS 穩定修復版)
# 💡 更新內容： 強制鎖定 114年(25Q4)，完美相容表單括號標題
# ==========================================
...
# ==========================================
# 🌟 直接寫死，不讓伺服器時間干擾！
TARGET_YEAR_ROC = "114"   # 官方的民國 114 年 (即 2025 年)
TARGET_Q = 4              # 抓取 Q4 累計資料
Q_STRING = "25Q4"         # 尋找表單裡的 "25Q4"
COL_NAME_CUM_EPS = "最新累季" # 只要包含這四個字就能找到
# ==========================================
...
def fetch_and_update():
    print(f"啟動財報更新機器人：鎖定抓取【{TARGET_YEAR_ROC}年 Q{TARGET_Q}】資料 (尋找表單標題: {Q_STRING})...")
    ...
    for ws in target_sheets:
        ...
        # 💡 只要標題「包含」25Q4單季每股盈餘，後面有(元)也會被精準抓到！
        ...
        print(f"\n🔍 檢查分頁: {ws.title}")
        if i_e != -1: print(f"   ✅ 成功找到單季EPS欄位: {h[i_e]}")
        else: print(f"   ❌ 找不到: {Q_STRING}單季每股盈餘")
        
        if i_ae != -1: print(f"   ✅ 成功找到累季EPS欄位: {h[i_ae]}")
        else: print(f"   ❌ 找不到包含: {COL_NAME_CUM_EPS} 的欄位")
