...
st.title("📊 2026 戰略指揮 (V160 記憶洗淨與防崩潰版)")

# 💡 V160 核心防護：相容性重整機制與雙重快取清除
def force_rerun():
    try:
        st.rerun()
                        ...
                        if i_c != -1 and i_e != -1:
                            ...
                            for r, row in enumerate(data):
                                ...
                                if code in curr_dict:
                                    ...
                                    if curr["rev"] > 0:
                                        # 🛡️ 絕對防禦機制：如果 API 沒給毛利(0)，機器人絕對不寫入，原封不動保留您表單裡的資料！
    ...
    with t_vip:
        ...
        if "df_vip" in st.session_state:
            ...
            with c1:
                ...
                # 💡 V160 完美修復：使用 dataframe 擷取，防止格式轉換錯誤崩潰！
