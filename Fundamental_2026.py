...
st.title("📊 股市戰略指揮中心 (V21 視覺強化版)")
st.markdown("💡 **專屬直連 ＆ 視覺升級：** 完美解析 CSV 報表，並升級「同期並列柱狀圖」，季成長動能一眼看穿！")
...
use_yahoo_price = st.sidebar.checkbox("🌐 連線 Yahoo 抓即時股價 (不勾選用表單股價)", value=False)
...
if uploaded_file is not None:
    try:
        ...
        st.session_state["stock_db_v21"] = stock_db
    ...
    if st.button(f"🚀 開始執行 {simulated_month} 月份戰略分析", type="primary"):
        with st.spinner("正在執行 VBA 核心運算，請稍候..."):
            ...
            for code, data in st.session_state["stock_db_v21"].items():
                ...
                results.append(res)
            st.session_state["df_final_v21"] = pd.DataFrame(results)
            st.success("✅ 分析完成！")

if "df_final_v21" in st.session_state:
    df = st.session_state["df_final_v21"]
    
    st.divider()
    st.subheader("📊 季營收動能對比 (去年度實際 vs 今年度預估)")
    selected_stock = st.selectbox("📌 請選擇要查看的個股：", sorted(df["股票名稱"].tolist()))
    
    stock_row = df[df["股票名稱"] == selected_stock].iloc[0]
    
    # 💡 視覺大變身：改用 Streamlit 內建的柱狀圖 (Bar Chart)，完美呈現同期並列！
    chart_data = pd.DataFrame({
        "去年度實際(億)": stock_row["_ly_qs"],
        "今年度預估(億)": stock_row["_est_qs"]
    }, index=["Q1", "Q2", "Q3", "Q4"])
    ...
    format_dict = {"最新股價": "{:.2f}", "當季預估均營收": "{:.2f}", "季成長率(YoY)%": "{:.2f}%", "預估今年Q1_EPS": "{:.2f}", "預估今年度_EPS": "{:.2f}", "本益比(PER)": "{:.2f}", "運算配息率(%)": "{:.2f}%", "前瞻殖利率(%)": "{:.2f}%"}
