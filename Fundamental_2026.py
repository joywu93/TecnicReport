            ...
            if "google_key" not in st.secrets: st.error("❌ 找不到金鑰！")
                ...
                with st.status("啟動 V135 終極清洗引擎...", expanded=True) as status:
                    try:
                        ...
                        # 💡 V135 防空包彈升級：如果抓到的值是空的，不要馬上 return，繼續找下一個符合的欄位！
                        ...
                        if not curr_dict:
                            ...
                            st.write("2️⃣ 啟動透視防護！正在匹配欄位並清除殘舊季增數...")
                            scopes = ['https://www.googleapis.com/auth/spreadsheets']
                            creds = Credentials.from_service_account_info(json.loads(st.secrets["google_key"]) if isinstance(st.secrets["google_key"], str) else dict(st.secrets["google_key"]), scopes=scopes)
                            client = gspread.authorize(creds)
                            worksheets = client.open_by_url(MASTER_GSHEET_URL).worksheets()
                                        ...
                                        if code in curr_dict:
                                            ...
                                            if curr["rev"] > 0:
                                                if idx_gm != -1:
                                                    ...
                                                    if idx_gm_qoq != -1: cells_to_update.append(gspread.Cell(row=row_i+1, col=idx_gm_qoq, value="")) # 清空季增數
                                                if idx_om != -1:
                                                    om = (curr["op"] / curr["rev"]) * 100
                                                    cells_to_update.append(gspread.Cell(row=row_i+1, col=idx_om, value=round(om, 2)))
                                                    if idx_om_qoq != -1: cells_to_update.append(gspread.Cell(row=row_i+1, col=idx_om_qoq, value="")) # 清空季增數
                            ...
                            status.update(label=f"🎉 V135 終極避坑版完成！已成功修復多欄位空包彈陷阱，共更新 {total_cells_updated} 個儲存格！", state="complete", expanded=False)
                            st.cache_data.clear() 
                            st.balloons()
                    except Exception as e:
                        status.update(label="任務中斷", state="error", expanded=True)
...
if cached_data:
    ...
    with tab_vip:
        if st.button(f"🚀 執行戰略分析", type="primary"):
            ...
            if found_count == 0:
                ...
                st.session_state["df_final_v135"] = pd.DataFrame(results)

        if "df_final_v135" in st.session_state:
            df = st.session_state["df_final_v135"].copy()
    ...
    if tab_radar is not None:
        with tab_radar:
            ...
            if st.button("📡 啟全市場掃描", type="primary", use_container_width=True):
