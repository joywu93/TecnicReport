import os, gspread, json, re, smtplib, time
import pandas as pd
...
def analyze_strategy(df, t):
    try:
        ...
        if (curr_p - prev_p)/prev_p >= 0.05 and curr_v > prev_v * 1.5: msg.append(f"🔥 強勢反彈：慎防跌破 {p3_close:.1f}"); is_mail = True
...
def smart_delay():
    now = datetime.utcnow()
    if 10 <= now.minute < 30:
        wait_sec = (30 - now.minute) * 60 - now.second
        print(f"⏳ 等待 {int(wait_sec)} 秒至 30 分精準抓取...")
        time.sleep(wait_sec)
    elif 40 <= now.minute < 60:
        wait_sec = (60 - now.minute) * 60 - now.second
        print(f"⏳ 等待 {int(wait_sec)} 秒至整點精準抓取...")
        time.sleep(wait_sec)
...
def run_batch():
    try:
        ...
        for row in sheet.get_all_records():
            email = row.get('Email')
            # 💡 擴充正則，抓取 4位數字 與 ^TWII
            tickers = [t.upper() for t in re.findall(r'\d{4}|\^TWII', str(row.get('Stock_List', '')), re.IGNORECASE)]
            if not email: continue
            
            notify_list = [f"✅ 股市戰略偵察機連線成功！精準抓取時間：{datetime.now().strftime('%H:%M:%S')}"]
            for t in tickers:
                # 💡 針對大盤 ^TWII 不加 .TW
                if t == "^TWII":
