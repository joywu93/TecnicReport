...
# --- 💡 智能守候函數 (解決提早排隊資料不準的問題) ---
def smart_delay():
    now = datetime.utcnow()
    # 情況 1：如果是 09:15 ~ 09:29 之間喚醒，等待到 09:30
    if 10 <= now.minute < 30:
        wait_sec = (30 - now.minute) * 60 - now.second
        print(f"⏳ 提早排隊搶到伺服器！等待 {int(wait_sec)} 秒，直到 30 分整才精準抓取數據...")
        time.sleep(wait_sec)
    # 情況 2：如果是 12:45 ~ 12:59 之間喚醒，等待到 13:00 (整點)
    elif 40 <= now.minute < 60:
        wait_sec = (60 - now.minute) * 60 - now.second
        print(f"⏳ 提早排隊搶到伺服器！等待 {int(wait_sec)} 秒，直到整點才精準抓取數據...")
        time.sleep(wait_sec)
